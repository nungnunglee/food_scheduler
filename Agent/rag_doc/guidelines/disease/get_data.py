...
# image_path = "https://chs.kdca.go.kr/cscdnhfile/health/healthNewDown/healthInfoFileDown.do?SEQ=17e8a6fc67b1"
# image_bytes = requests.get(image_path, verify=False).content
# image = types.Part.from_bytes(
#   data=image_bytes, mime_type="image/jpeg"
# )

# client = genai.Client(api_key="AIzaSyAe311XYoxfodXWaJoV--B2B-sVxuXcSMo")
# response = client.models.generate_content(
#     model="gemini-2.0-flash-exp",
#     contents=["이 이미지를 자세히 설명해 주세요.", image],
# )
...
# def xml_to_html(xml_file_path, output_dir='output'):
#     """
#     Convert XML file to HTML with proper formatting
...
#     svc = root.find('svc')
#     if svc is None:
#         raise ValueError("No 'svc' element found in XML")
...
#     # Add title
#     title = svc.find('CNTNTSSJ')
#     if title is not None:
#         html_content += f"<h1>{title.text.strip()}</h1>"
    
#     # Process each content section
#     cntnts_cl_list = svc.find('cntntsClList')
#     if cntnts_cl_list is not None:
#         for cntnts_cl in cntnts_cl_list.findall('cntntsCl'):
#             section_name = cntnts_cl.find('CNTNTS_CL_NM')
#             section_content = cntnts_cl.find('CNTNTS_CL_CN')
            
#             if section_name is not None and section_content is not None:
#                 html_content += f"""
#                 <div class="section">
#                     <h2>{section_name.text.strip()}</h2>
...
#     # Save HTML file
#     output_file = os.path.join(output_dir, os.path.splitext(os.path.basename(xml_file_path))[0] + '.html')
...
#     # Images
#     html_content = re.sub(r'<img.*?src="(.*?)".*?alt="(.*?)".*?>', r'![alt](\1)', html_content)
...
# def xml_to_markdown(xml_file_path, output_dir='output'):
#     """
#     Convert XML file to Markdown with proper formatting
...
#     title = svc.find('CNTNTSSJ')
#     if title is not None:
#         markdown_content += f"# {title.text.strip()}\n\n"
...
#             if section_name is not None and section_content is not None:
#                 markdown_content += f"## {section_name.text.strip()}\n\n"
#                 # Convert HTML content to Markdown
#                 markdown_content += html_to_markdown(section_content.text.strip()) + "\n\n"
    
#     # Save Markdown file
#     output_file = os.path.join(output_dir, os.path.splitext(os.path.basename(xml_file_path))[0] + '.md')
#     with open(output_file, 'w', encoding='utf-8') as f:
...
# if __name__ == "__main__":
#     # Example usage
#     xml_file = "Agent/rag_doc/guidelines/disease/변비.xml"
#     output_html = xml_to_html(xml_file)
#     output_md = xml_to_markdown(xml_file)
